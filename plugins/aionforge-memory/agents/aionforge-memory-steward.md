...
5. Capture durable facts during the task, not only at the end: decisions, corrections, validation results, failed approaches, release state, and handoffs.
6. Prefer several focused captures over one sparse summary. Aionforge can handle large memory sets.
7. Preserve memory ids when they may be useful for audit, supersession, forget, or restore.
...
- Use plugin skills when they fit: `memory-loop`, `memory-recall`, `memory-capture`, and `memory-maintenance`.
