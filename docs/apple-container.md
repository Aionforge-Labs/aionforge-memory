...
Apple's `container` runtime can build and run the same OCI image published for
Aionforge Memory. It is a local macOS path for Apple silicon machines; release
publishing still uses GHCR and the existing Docker/buildx workflow.
...
container build --arch arm64 --tag aionforge-memory:dev .
container run -d \
